...
const cookingMethods = [
  "Baked", "Steamed", "Boiled", "Sautéed", "Fried",
  "Grilled", "Wood-fired", "Slow-cooked",
];
const categoryOptions = ["Pizza", "Extras", "Sides", "Drinks", "Desserts"];
...
export default function PizzaCreator() {
  ...
  const [form, setForm] = useState({
    ...
    cookingMethod: "", imageFile: null, ingredients: [],
  });
  ...
  const addIngredient = () => {
    const qty = {}; sizeList.forEach(s => qty[s] = 0);
    setForm(p => ({
      ...p,
    ...
    <>
      ...
      <form className="pizza-form" onSubmit={onSubmit}>
        <div className="pc-grid">
          ...
          <label>
            Cooking Method
            <select name="cookingMethod" value={form.cookingMethod} onChange={onChange}>
              <option value="">– choose –</option>
              {cookingMethods.map(m => <option key={m}>{m}</option>)}
            </select>
          </label>
      ...
      <div className="table-wrapper">
        <table className="ing-table">
          <thead>
            <tr>
              <th>Delete</th>
              <th onClick={toggleSortByName} style={{cursor:"pointer", userSelect:"none"}}>
                Name {sortAsc ? "▲" : "▼"}
              </th>
              <th>Category</th>
              <th>Sizes</th>
          ...
          <tbody>
            {pizzas
              ...
              .map(p => (
                <tr key={p.id}>
                  <td>
                    <button className="del-btn" onClick={() => deletePizza(p.id)}>✕</button>
                  </td>
                  <td>{p.name}</td>
                  <td>{p.category}</td>
